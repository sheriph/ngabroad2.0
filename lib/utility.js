...
import { forEach, get, trim, truncate } from "lodash";
...
export const useUser = () => {
  const userFetcher = async () => {
    try {
      ...
      const user = await axios.post("/api/others/getuserdata", { email });
...
export const useAuthUser = (userExist) => {
  const userFetcher = async () => {
    try {
      // console.log("geting authed user");
  ...
  const {
    ...
    isValidating,
    isLoading,
  } = useSWR(userExist ? "/useAuthUser" : undefined, userFetcher);

  return { user, isLoading, error, mutate, isValidating };
};
...
export const getCommentCount = async (key) => {
  try {
    const { post_id } = JSON.parse(key);
...
export const revalidateToken = async () => {
  ...
  try {
    ...
    console.log("error.response.data cookie error", error.response.data);
  }
};
...
export const viewErrors = (errors) => {
  forEach(errors || [], (error, index) => {
    toast.error(
      <React.Fragment>
        <Typography>{titleCase(get(error, "title", ""))}</Typography>
        <Typography variant="caption">
          {titleCase(get(error, "detail", ""))}
        </Typography>
      </React.Fragment>
    );
  });
...
export const viewMongoError = (message) => {
  console.log("message", message);
  toast.error(
    <React.Fragment>
      <Typography variant="caption">{titleCase(message)}</Typography>
    </React.Fragment>
  );
};
...
export const makePayment = (
  ...
  const modal = FlutterwaveCheckout({
    ...
    tx_ref: `ngabroad-${Math.random().toString(36).slice(2, 20)}`,
    amount: amount,
    currency: "NGN",
    ...
    callback: async (payment) => {
      ...
      try {
        await toast.promise(
          axios.post("/api/payment/verifypayment", {
            reference,
          ...
          }),
          {
            ...
            success: "Payment verified succesfuly",
          }
        );
        await axios.post("/api/visa/paymentemail", {
          passengerData,
          visaOrderParams,
          reference,
        });
        mutate();
        // router.push(redirect_url).then((res) => modal.close());
    ...
    onclose: function (incomplete) {
      ...
      if (incomplete === true) {
        ...
        // router.push(redirect_url).then((res) => modal.clos());
      }
    },
