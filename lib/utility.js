...
export const getUsername = async (user_id) => {
  try {
    const username = await axios.post("/api/others/getusername", { user_id: user_id });
...
/* export const userFetcher = async () => {
  try {
    ...
    const user = await axios.post("/api/others/getuserdata", {
...
export const useFetchPosts = (filter) => {
  const fetchPosts = async (key) => {
    try {
      ...
      const posts = await axios.post("/api/others/getposts", { ...dbFilter });
